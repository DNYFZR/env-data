...
def extract_api_data( start_date = dt.date(2022,2,20), end_date = dt.date(2022,2,22) ):
    '''
...
def extract_json_data(dataset = extract_api_data()):
    # Set up columns
...
def transform_json(table = extract_json_data()):
    # Set up containers
    output = pd.DataFrame()
...
def transform_json_energy(table = transform_json()):
...
def run_pipeline(start, end):
    api_call = extract_api_data(start_date= start, end_date= end)
    json_extract = extract_json_data(dataset= api_call)
    json_transform = transform_json(table= json_extract)
    json_transform_energy = transform_json_energy(table= json_transform)
    
    return json_transform_energy

if __name__ == '__main__':
    from time import time
    run_start = time()
    dataset = run_pipeline(start = dt.date(2022, 1, 1), end = dt.date(2022, 4, 1))
